...
- What
  ...
  <img src="https://github.com/lisaalaz/papers/blob/master/images/ToT_schematic.png?raw=true" width="800">
  ...
  - Architecture
    ...
    - To instantiate ToT we need:
      ...
      2) A __thought generator__ to generate potential thoughts from each state. The generator is a function $G(p_\theta, s, k)$ which given a state $s$ can generate $k$ candidates for the next thought steps in two ways:
         - Sampling i.i.d. thoughts from a chain-of-thought prompt $z^(j) ∼ p_\theta^{CoT}(z_{i+1}|s) = p_\theta^{CoT}(z_{i+1}|x, z_{1...i})(j = 1 ... k)$. This works well when the thought space is rich (e.g. each thought is a paragraph) and i.i.d. samples lead to diversity. <img src="https://github.com/lisaalaz/papers/blob/master/images/ToT_creative_writing_setup.png?raw=true" width="800">
